...
const { document, XMLHttpRequest } = new jsdom.JSDOM("<body></body>", {
  resources: "usable",
  url: `file://${__dirname}/../`,
}).window;
...
var CBL = function (options) {
  var defaults = {
    preprocess: function () {
      warn("You should define a preprocess method!");
    },
    model_file: "",
    model_string: "",
  ...
  var obj = {
      ...
      addQueue(function () {
        ...
        var afterLoad = function () {
          ...
          if (options.fixed_blob_locations.length > 0) {
            ...
            blobs = cblImage.segmentBlobs(
              options.blob_min_pixels,
    ...
    loadNextPattern: function () {
      ...
      if (nextPattern) {
        ...
        document.getElementById(nextPattern.txtId).onkeyup = function (event) {
          ...
          if (
            (options.character_set.indexOf(typedLetter) > -1 &&
              typedLetter.length) ||
            typedLetter == options.incorrect_segment_char
          ) {
    ...
    visualizeModel: function (elementId) {
      for (var m = 0; m < model.length; m++) {
        ...
        var pctx = pattern
          .getContext("2d")
          .getImageData(0, 0, options.pattern_width, options.pattern_height);
    ...
    condenseModel: function () {
        ...
        for (var j = 0; j < newModel.length; j++) {
          ...
          if (newModel[j].solution == model[i].solution) {
            for (var x = 0; x < newModel[j].tempArray.length; x++) {
              newModel[j].tempArray[x] =
                parseInt(newModel[j].tempArray[x]) + parseInt(patternArray[x]);
            }
      ...
      for (var i = 0; i < newModel.length; i++) {
        for (var x = 0; x < newModel[i].tempArray.length; x++) {
          newModel[i].tempArray[x] = Math.round(
            newModel[i].tempArray[x] / newModel[i].tempCount
          );
        }
    ...
    var obj = {
      ...
      removeGray: function (tolerance) {
        ...
        for (var x = 0; x < image.width; x++) {
          for (var y = 0; y < image.height; y++) {
            var i = x * 4 + y * 4 * image.width;
            var diff = Math.max(
              Math.abs(image.data[i] - image.data[i + 1]),
          ...
          for (var x = 0; x < image.width - 1; x++) {
            ...
            for (var cy = 0; cy < w; cy++) {
              for (var cx = 0; cx < h; cx++) {
                var cpx =
                  ((y + (cy - half)) * image.width + (x + (cx - half))) * 4;
                r +=
                  image.data[(cpx + image.data.length) % image.data.length] *
                  matrix[cy][cx];
                g +=
                  image.data[
                    (cpx + 1 + image.data.length) % image.data.length
                  ] * matrix[cy][cx];
                b +=
                  image.data[
                    (cpx + 2 + image.data.length) % image.data.length
                  ] * matrix[cy][cx];
              }
            }
      ...
      removeHorizontalLine: function (lineWidth, color) {
        ...
        var search = function (x, y, line) {
          ...
          if (
            x >= canvas.width - 1 &&
            ends.some(function (e) {
              return (
                e.x == copy.points[copy.points.length - 1].x &&
                e.y == copy.points[copy.points.length - 1].y
              );
            })
          ) {
            return copy;
          ...
          for (var d = 0; d < play.length; d++) {
            if (y + play[d] >= 0 && y + play[d] < canvas.height) {
              ...
              if (
                dead.some(function (e) {
                  return e.x == x + 1 && e.y == y + play[d];
                })
              ) {
          ...
          return allLines && allLines.length
            ? allLines.reduce(function (a, b) {
                return a.vertical < b.vertical ? a : b;
              })
            : null;
        };
        // Remove the lines
        for (var s = 0; s < starts.length; s++) {
          var line = search(starts[s].x, starts[s].y, {
            points: [],
            vertical: 0,
          });
          if (line && line.points) {
            for (var p = 0; p < line.points.length; p++) {
              for (var w = -lineWidth; w <= lineWidth; w++) {
                var i =
                  line.points[p].x * 4 +
                  (line.points[p].y + w) * 4 * image.width;
                var k =
                  line.points[p].x * 4 +
                  (line.points[p].y + w - 1) * 4 * image.width;
      ...
      segmentBlocks: function (
        ...
        for (var c = 0; c < segmentLocations.length; c++) {
          ...
          if (options.pattern_maintain_ratio) {
            ...
            blob
              .getContext("2d")
              .drawImage(temp, 0, 0, segmentWidth, segmentHeight);
          }
      ...
      segmentBlobs: function (
        ...
        var image = canvas
          .getContext("2d")
          .getImageData(0, 0, canvas.width, canvas.height);
        var toColor = function (d, i) {
          return d[i] * 255 * 255 + d[i + 1] * 256 + d[i + 2];
        };
        ...
        if (options.exact_characters > 0) {
          ...
          while (blobs.length < options.exact_characters) {
            ...
            if (options.exact_characters_width > 0) {
              resultingBlobs = Math.ceil(
                largest / options.exact_characters_width
              );
              resultingBlobs = Math.max(resultingBlobs, 2);
              resultingBlobs = Math.min(
                resultingBlobs,
                options.exact_characters - blobs.length + 1
              );
            }

            for (var split = 1; split <= resultingBlobs; split++) {
              var splitSection = cloneCanvas(blobs[largestIndex].orig_image);
              var blobContext = splitSection
                .getContext("2d")
                .getImageData(0, 0, splitSection.width, splitSection.height);
              ...
              if (options.exact_characters_play > 0) {
                ...
                var bestLeftX = 0;
                for (
                  var fpx = leftmost - options.exact_characters_play;
                  fpx < leftmost + options.exact_characters_play;
                  fpx++
                ) {
                  var currentLeft = 0;
                ...
                var bestRightX = 0;
                for (
                  var fpx = rightmost + options.exact_characters_play;
                  fpx > rightmost - options.exact_characters_play;
                  fpx--
                ) {
                  var currentRight = 0;
                  for (var fpy = topmost; fpy < bottommost; fpy++) {
              ...
              for (
                var fpy = bottommost - 1;
                fpy > topmost && bottommost == splitSection.height;
                fpy--
              ) {
                for (
                  var fpx = leftmost;
                  fpx < rightmost && bottommost == splitSection.height;
                  fpx++
                ) {
                  var fpix = getPixel(blobContext, fpx, fpy);
                  if (fpix.r != 255 || fpix.g != 255 || fpix.b != 255) {
  ...
  var blobToPattern = function (blob) {
    ...
    var image = blob
      .getContext("2d")
      .getImageData(0, 0, blob.width, blob.height);
    for (var x = 0; x < image.width; x++) {
      for (var y = 0; y < image.height; y++) {
        var i = x * 4 + y * 4 * image.width;
  ...
  var pixelCompareAndSet = function (
    i,
    targetcolor,
    targettotal,
    fillcolor,
  ...
  var colorCompareMaxRGB = function (color1, color2) {
    return Math.sqrt(
      ...
      Math.pow(color1.g - color2.g, 2)) / 3
    );
  };
    ...
    if (document.getElementById("cbl-trainer") != null) {
      ...
      appendHtml(
        ...
        '<div id="cbl-trainer">' +
          ...
          '        <p>Type <span class="cbl-discard">' +
          options.incorrect_segment_char +
          "</span> to discard a pattern if the image was not segmented properly.</p>" +
          '        <div class="cbl-row">' +
